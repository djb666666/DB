...
public class SysUserServiceImpl implements SysUserService{
	 ...
	 * @Override
	 * 
	 * public int updatePassword(String password, String newPassword, String
	 * cfgPassword) { if(StringUtils.isEmpty(newPassword)) throw new
	 * IllegalArgumentException("新密码不能为空"); if(StringUtils.isEmpty(password)) throw
	 * new IllegalArgumentException("原密码不能为空"); if(StringUtils.isEmpty(cfgPassword))
	 * throw new IllegalArgumentException("确认密码不能为空");
	 * if(!cfgPassword.equals(newPassword)) throw new
	 * IllegalArgumentException("两次密码输入不一致"); SysUser user = (SysUser)
	 * SecurityUtils.getSubject().getPrincipal(); SimpleHash sh=new
	 * SimpleHash("MD5", password, user.getSalt(), 1);
	 * if(!user.getPassword().equals(sh.toHex())) throw new
	 * IllegalArgumentException("原密码不正确"); String salt=UUID.randomUUID().toString();
	 * sh=new SimpleHash("MD5",newPassword,salt,1); int
	 * rows=sysUserDao.updatePassword(sh.toHex(), salt,user.getId()); if(rows==0)
	 * throw new ServiceException("修改失败"); return rows; }
	 */
